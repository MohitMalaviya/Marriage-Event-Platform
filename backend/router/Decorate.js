...
const Decorate = require("../model/Decorate");
...
router.post("/adddecorate", upload, async (req, res) => {
    ...
    if (
        ...
        const data = await Decorate.insertMany({
...
router.post("/updatedecorate", upload, async (req, res) => {
    // const is_check = await Decorate.find().sort({ _id: -1 }).limit(1)
    // const id = is_check.length ? is_check[0].p_id.slice(1) : 0
    // const p_id = "P" + (Number(id) + 1)
...
router.post('/getdecorate', async (req, res) => {
    const result = await Decorate.find();
    if (result.length) {
        res.json({
            status: true,
...
router.post('/getdecoratebyid', async (req, res) => {
    const { p_id } = req.body
    if (!p_id) {
        res.send({
            status: false,
        ...
        const result = await Decorate.find({ p_id: p_id });
        if (result.length) {
            res.json({
                status: true,
                message: "Data get successfully",
...
router.delete('/deletedecorate', async (req, res) => {
    const { p_id } = req.body
    if (!p_id) {
        res.send({
            status: false,
        ...
        const result = await Decorate.find({ p_id: p_id });
        if (result.length) {
            await Decorate.deleteMany({ p_id: p_id });
            res.json({
                status: true,
